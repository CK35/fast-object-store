...
public class TasksConfiguration implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		if(env.getProperty(PropPrefix.join("tasks.skip"), Boolean.class, false)) {
			return;
		}
		taskScheduler().schedule(compressTask(), new CronTrigger(env.getProperty(PropPrefix.join("tasks.compress.cron"), DEFAULT_COMPRESS_CRON)));
		taskScheduler().schedule(deleteTask(), new CronTrigger(env.getProperty(PropPrefix.join("tasks.delete.cron"), DEFAULT_DELETE_CRON)));
	}
	
	@Bean
	public MinimumIntSetting maxUncompressedDaysSetting() {
		return new MinimumIntSetting(DEFUALT_MAX_UNCOMPRESSED_DAYS, 0, env.getProperty(PropPrefix.join("tasks.compress.max.uncompressed.days"), Integer.class));
	}
	
	@Bean
	public MinimumIntSetting maxDaysToKeepSetting() {
		return new MinimumIntSetting(DEFAULT_MAX_DAYS_TO_KEEP, 0, env.getProperty(PropPrefix.join("tasks.delete.max.days.to.keep"), Integer.class));
	}
